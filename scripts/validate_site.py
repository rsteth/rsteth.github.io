...
def resolve_site_path(target: str) -> Path:
    ...
    try:
        ...
        raise ValueError(f"{target} must stay within the site root.") from exc
    return resolved


def main() -> None:
    required_files = [INDEX_FILE, SITE_DIR / "styles.css", SITE_DIR / "script.js", SITE_DIR / "image.gif", SITE_DIR / "CNAME"]
    ...
    if not "".join(parser.title).strip():
        raise SystemExit("index.html must include a non-empty <title>.")
    if not parser.has_h1:
        raise SystemExit("index.html must include an <h1>.")
    ...
    for target in parser.local_links:
        ...
        if not asset_path.exists():
            missing_assets.append(target)
        elif asset_path.is_dir() and not (asset_path / "index.html").exists():
            missing_assets.append(target)
    if missing_assets:
        raise SystemExit(f"index.html references missing local assets: {', '.join(sorted(set(missing_assets)))}")

    cname = (SITE_DIR / "CNAME").read_text(encoding="utf-8").strip()
    if not cname or "." not in cname:
        raise SystemExit("CNAME must contain the configured domain.")
